...
namespace nfs2iso2nfs
{
    class Program
    {
        ...
        public static string isoFile = "." + Path.DirectorySeparatorChar + "game.iso";
        public static string wiiKeyFile = "." + Path.DirectorySeparatorChar + "wii_common_key.bin";
        public static string nfsDir = "." + Path.DirectorySeparatorChar + "content";
        public static string fw_file = "." + Path.DirectorySeparatorChar + "code" + Path.DirectorySeparatorChar + "fw.img";
        ...
        static void Main(string[] args)
        {
            ...
            if (dec)
            {
                byte[] header = getHeader(nfsDir + Path.DirectorySeparatorChar + "hif_000000.nfs");
        ...
        public static int checkArgs(string[] args)
        {
            for (int i = 0; i < args.Length; i++)
                switch (args[i])
                {
                    ...
                    case "-help":
                        ...
                        Console.WriteLine("-key <file>     Location of AES key file. DEFAULT: ." + Path.DirectorySeparatorChar + "code" + Path.DirectorySeparatorChar + "htk.bin.");
                        Console.WriteLine("-wiikey <file>  Location of Wii Common key file. DEFAULT: ." + Path.DirectorySeparatorChar + "wii_common_key.bin.");
                        Console.WriteLine("-iso <file>     Location of .iso file. DEFAULT: ." + Path.DirectorySeparatorChar + "game.iso.");
                        Console.WriteLine("-nfs <file>     Location of .nfs files. DEFAULT: ." + Path.DirectorySeparatorChar + "content.");
                        Console.WriteLine("-fwimg <file>   Location of fw.img. DEFAULT: ." + Path.DirectorySeparatorChar + "code" + Path.DirectorySeparatorChar + "fw.img.");
                        Console.WriteLine();
            ...
            if (!Path.IsPathRooted(nfsDir))
                nfsDir = dir + Path.DirectorySeparatorChar + nfsDir;
            if (!Path.IsPathRooted(fw_file))
                fw_file = dir + Path.DirectorySeparatorChar + fw_file;
            ...
            if (dec || ((!dec && !enc) && File.Exists(nfsDir + Path.DirectorySeparatorChar + "hif_000000.nfs")))
            {
                Console.WriteLine("+++++ NFS2ISO +++++");
                Console.WriteLine();
                if (dec && !enc && !File.Exists(nfsDir + Path.DirectorySeparatorChar + "hif_000000.nfs"))
                {
                    Console.WriteLine("ERROR: .nfs files not found! Exiting...");
                    return -1;
                }
                else if ((!dec && !enc) && File.Exists(nfsDir + Path.DirectorySeparatorChar + "hif_000000.nfs"))
                {
        ...
        public static void combineNFSFiles(string outFile)
        {
            using (var nfs = new BinaryWriter(File.OpenWrite(outFile)))
            {
                ...
                int nfsNo = -1;
                while (File.Exists(nfsDir + Path.DirectorySeparatorChar + "hif_" + String.Format("{0:D6}", nfsNo + 1) + ".nfs"))
                    nfsNo++;
                ...
                for (int i = 0; i <= nfsNo; i++)
                {
                    ...
                    var nfsTemp = new BinaryReader(File.OpenRead(nfsDir + Path.DirectorySeparatorChar + "hif_" + String.Format("{0:D6}", i) + ".nfs"));
                    if (i == 0)
                    {
